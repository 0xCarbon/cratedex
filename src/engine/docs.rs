...
use rustdoc_types::{
    ...
    GenericArg, GenericArgs, GenericBound, GenericParamDef, GenericParamDefKind, ItemEnum, Path as RustdocPath,
    PolyTrait, Term, TraitBoundModifier, Type, WherePredicate,
};
...
pub fn ensure_docs_table_and_fts(conn: &Connection) -> AppResult<()> {
    ...
    let needs_recreate = {
        let mut stmt = conn.prepare(
            "SELECT count(*) FROM pragma_table_info('docs') WHERE name = 'signature'",
        )?;
        ...
        let mut stmt2 = conn.prepare(
            "SELECT count(*) FROM pragma_table_info('docs')",
        )?;
    ...
    match ty {
        ...
        Type::QualifiedPath { name, args, self_type, trait_ } => {
            ...
            let mut s = if let Some(trait_path) = trait_ {
                format!("<{self_rendered} as {}>::{name}", render_path(trait_path, d))
            } else {
        ...
        Type::Pat { type_, __pat_unstable_do_not_use } => {
    ...
    match bound {
        GenericBound::TraitBound { trait_, generic_params, modifier } => {
            ...
            if !generic_params.is_empty() {
                write!(s, "for<{}> ", render_generic_param_defs(generic_params, depth)).unwrap();
            }
            match modifier {
        ...
        GenericBound::Use(args) => {
            let parts: Vec<_> = args.iter().map(|a| match a {
    ...
    let parts: Vec<String> = predicates
        ...
        .map(|p| match p {
            WherePredicate::BoundPredicate { type_, bounds, generic_params } => {
                ...
                write!(s, "{}: {}", render_type(type_, depth), render_generic_bounds(bounds, depth)).unwrap();
                s
            }
...
fn render_poly_trait(pt: &PolyTrait, depth: usize) -> String {
    ...
    if !pt.generic_params.is_empty() {
        write!(s, "for<{}> ", render_generic_param_defs(&pt.generic_params, depth)).unwrap();
    }
...
fn render_fn_pointer(fp: &FunctionPointer, depth: usize) -> String {
    ...
    if !fp.generic_params.is_empty() {
        write!(s, "for<{}> ", render_generic_param_defs(&fp.generic_params, depth)).unwrap();
    }
...
fn render_abi(abi: &Abi) -> String {
    match abi {
        Abi::Rust => String::new(),
        Abi::C { unwind } => if *unwind { "extern \"C-unwind\" ".into() } else { "extern \"C\" ".into() },
        Abi::System { unwind } => if *unwind { "extern \"system-unwind\" ".into() } else { "extern \"system\" ".into() },
...
fn render_function_signature(func: &rustdoc_types::Function, name: &str) -> String {
    ...
    let real_params: Vec<_> = func.generics.params.iter()
        .filter(|p| !matches!(&p.kind, GenericParamDefKind::Type { is_synthetic: true, .. }))
        .collect();
    if !real_params.is_empty() {
        write!(s, "<{}>", real_params.iter().map(|p| render_generic_param_def(p, 0)).collect::<Vec<_>>().join(", ")).unwrap();
    }
    ...
    let params: Vec<_> = func.sig.inputs.iter().map(|(name, ty)| {
        ...
        if name == "self" {
            ...
            match ty {
                Type::BorrowedRef { lifetime, is_mutable, .. } => {
...
fn render_struct_signature(strct: &rustdoc_types::Struct, name: &str) -> String {
    ...
    s.push_str(name);
    let real_params: Vec<_> = strct.generics.params.iter()
        .filter(|p| !matches!(&p.kind, GenericParamDefKind::Type { is_synthetic: true, .. }))
        .collect();
    if !real_params.is_empty() {
        write!(s, "<{}>", real_params.iter().map(|p| render_generic_param_def(p, 0)).collect::<Vec<_>>().join(", ")).unwrap();
    }
    s.push_str(&render_where_predicates(&strct.generics.where_predicates, 0));
    s
}
...
fn render_enum_signature(enm: &rustdoc_types::Enum, name: &str) -> String {
    ...
    s.push_str(name);
    let real_params: Vec<_> = enm.generics.params.iter()
        .filter(|p| !matches!(&p.kind, GenericParamDefKind::Type { is_synthetic: true, .. }))
        .collect();
    if !real_params.is_empty() {
        write!(s, "<{}>", real_params.iter().map(|p| render_generic_param_def(p, 0)).collect::<Vec<_>>().join(", ")).unwrap();
...
fn render_trait_signature(trt: &rustdoc_types::Trait, name: &str) -> String {
    ...
    s.push_str("trait ");
    s.push_str(name);
    let real_params: Vec<_> = trt.generics.params.iter()
        .filter(|p| !matches!(&p.kind, GenericParamDefKind::Type { is_synthetic: true, .. }))
        .collect();
    if !real_params.is_empty() {
        write!(s, "<{}>", real_params.iter().map(|p| render_generic_param_def(p, 0)).collect::<Vec<_>>().join(", ")).unwrap();
...
fn render_type_alias_signature(ta: &rustdoc_types::TypeAlias, name: &str) -> String {
    ...
    s.push_str(name);
    let real_params: Vec<_> = ta.generics.params.iter()
        .filter(|p| !matches!(&p.kind, GenericParamDefKind::Type { is_synthetic: true, .. }))
        .collect();
    if !real_params.is_empty() {
        write!(s, "<{}>", real_params.iter().map(|p| render_generic_param_def(p, 0)).collect::<Vec<_>>().join(", ")).unwrap();
...
fn render_variant_signature(variant: &rustdoc_types::Variant, name: &str, krate: &Crate) -> String {
    match &variant.kind {
        ...
        rustdoc_types::VariantKind::Tuple(fields) => {
            let field_types: Vec<_> = fields.iter().map(|field_id| {
                field_id
                    .as_ref()
        ...
        rustdoc_types::VariantKind::Struct { fields, .. } => {
            let field_strs: Vec<_> = fields.iter().filter_map(|id| {
                ...
                match &item.inner {
                    ItemEnum::StructField(ty) => Some(format!("{field_name}: {}", render_type(ty, 0))),
                    _ => None,
                }
            }).collect();
...
fn resolve_self_type(ty: &Type, krate: &Crate) -> (Option<String>, Option<String>) {
    match ty {
        Type::ResolvedPath(path) => {
            let name = path.path.rsplit("::").next().unwrap_or(&path.path).to_string();
            let kind = krate.paths.get(&path.id).map(|summary| {
                format!("{:?}", summary.kind)
            });
            (Some(name), kind)
        }
...
fn extract_all_items(
    ...
    for item in krate.index.values() {
        let ItemEnum::Impl(impl_) = &item.inner else { continue };
        ...
        let trait_name = impl_.trait_.as_ref().map(|p| {
            p.path.rsplit("::").next().unwrap_or(&p.path).to_string()
        });

        for member_id in &impl_.items {
            let Some(member) = krate.index.get(member_id) else { continue };
            ...
            match &member.inner {
                ...
                ItemEnum::AssocType { generics, bounds, type_ } => {
                    ...
                    let real_params: Vec<_> = generics.params.iter()
                        .filter(|p| !matches!(&p.kind, GenericParamDefKind::Type { is_synthetic: true, .. }))
                        .collect();
                    if !real_params.is_empty() {
                        write!(sig, "<{}>", real_params.iter().map(|p| render_generic_param_def(p, 0)).collect::<Vec<_>>().join(", ")).unwrap();
                    }
                    if !bounds.is_empty() {
    ...
    async fn is_crate_indexed_returns_true_after_insert() {
        ...
        db.call(|conn| {
            conn.execute(
                INSERT_SQL,
                params![
                    "tokio", "1.0.0", "tokio_hash", "spawn", "Function",
                    None::<String>, None::<String>, None::<String>,
                    "fn spawn()", "Spawns a task.",
                ],
            )?;
            Ok(())
        })
    ...
    async fn fts_search_by_parent_name() -> AppResult<()> {
        ...
        db.call(|conn| {
            conn.execute(
                INSERT_SQL,
                params![
                    "std", "1.0.0", "std_hash", "insert", "Method",
                    "HashMap", "Struct", None::<String>,
